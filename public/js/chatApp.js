...
joinGroupFrom.addEventListener('submit',async(e)=>{
    ...
    const joinRes = await axios.get(`http://localhost:4000/message/joinGroup?gId=${tokenInput.value}`, { headers: {'Authorization': token}});
    ...
    if(isAd){
        ...
        if(presentUId !== ele.userId){
            ...
            remBtn.addEventListener('click', async () => {
                ...
                const removed = await axios.get(`http://localhost:4000/message/removeU?id=${ele.userId}&gId=${activeGroup}`, { headers: {'Authorization': token}});
                window.location.reload();
            })
        ...
        if(ele.isAdmin){
            ...
            makeAdmin.addEventListener('click', async ()=>{
                ...
                const admined = await axios.get(`http://localhost:4000/message/makeA?id=${ele.userId}&gId=${activeGroup}`, { headers: {'Authorization': token}});
                window.location.reload();
            })
...
fileInput.addEventListener('submit',async(e)=>{
    ...
    fetch("http://localhost:4000/message/saveFile", {
        method: 'POST',
        body: formData,
        headers: {
          "Authorization":token,
